...
if len(sys.argv) > 1:
	...
	Constants.SUMMARY_NAME = METHOD_NAME + NORM_NAME + \
							"_LAYER_" + str(Constants.LAYERS) + \
							"_LR_" + "{:.0e}".format(Constants.LEARNING_RATE) + "-" + \
							"{:.0e}".format(Constants.MIN_LEARNING_RT) + \
							"_F_" + str(Constants.NUM_FEATURES) + \
							"_IN_" + IN_RANK_NAME + \
							"_OUT_" + OUT_RANK_NAME + \
							"_RANGE_" + RANGE_NAME + \
							"_BATCH_" + str(Constants.BATCH_SIZE) + \
							"_EMBED_" + str(Constants.EMBED_SIZE) + \
							"_LAMBDA_" + str(Constants.LAMBDA_REGUL) + \
							"_TH_" + "{:.2f}".format(Constants.TH_CENT) + \
