...
class Graph():
	...
	#---------------------------------------------------------------------------
	def generate_random_geometry_graph(self, n, r):
		pos = {i: (random.uniform(0, 1), random.uniform(0, 1)) for i in range(n)}
		graphT=nx.random_geometric_graph(n, r, pos=pos)
		self.graph=nx.Graph()
		self.graph.add_nodes_from(graphT.nodes)
		self.graph.add_edges_from(graphT.edges)
